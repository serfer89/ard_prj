...
switch (payload[0])
   ...
   (payload[2] == '0' ) ? digitalWrite(12, LOW) : digitalWrite(12, HIGH);
        ...
        break;
    case '7':
   (payload[2] == '0' ) ? digitalWrite(13, LOW) : digitalWrite(13, HIGH);
                client.publish(room, answ);
                Serial.print("#7");
        break;

    case '8':
   (payload[2] == '0' ) ? digitalWrite(15, LOW) : digitalWrite(15, HIGH);
                client.publish(room, answ);
                Serial.print("#8");
        break;
    }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish(room_temp, "hello world");
      // ... and resubscribe
      client.subscribe(room);
      if (startup = 1) {
            client.publish(room, "startup");}
...
/*    if(analogRead(A0)>1)//если кнопка нажата ... 
    ...
    int state = digitalRead(02); //o/2/1/1
      Serial.print("state: ");
...
String sv = "o/2/1/"+ String(state);// operation ok
