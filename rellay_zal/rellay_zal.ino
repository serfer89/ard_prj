...
boolean flag=0;
char* room = "Зала";
char* room_temp="Зала/Температура";
unsigned long now;
int state = 0;
String ip;
...
switch (payload[0])
    {
   case '4':
   if (payload[2] == '0' ) { digitalWrite(16, LOW);} 
   ...
   (payload[2] == '0') ? digitalWrite(5, LOW) : digitalWrite(5, HIGH);
    ...
    case '6':
   (payload[2] == '0' ) ? digitalWrite(14, LOW) : digitalWrite(14, HIGH);
             client.publish(room, answ);
             Serial.print("#3");
            break;
    
        case '7':
   (payload[2] == '0' ) ? digitalWrite(0, LOW) : digitalWrite(0, HIGH);
             client.publish(room, answ);
             Serial.print("#4");
            break;
   
        case '8':
   (payload[2] == '0' ) ? digitalWrite(12, LOW) : digitalWrite(12, HIGH);
             client.publish(room, answ);
             Serial.print("#5");
            break;
...
bool reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish("Зала/Температура", "hello world");
      // ... and resubscribe
      client.subscribe("Зала");
            client.publish("Зала", "startup");
...
String sv = "s/2/1/"+ String(state);// dev state     
    ...
    Serial.println(temp);
    client.publish("Зала/Температура",  String(temp).c_str(),TRUE);
    client.publish("Зала2/ip",  ip.c_str(),TRUE);
