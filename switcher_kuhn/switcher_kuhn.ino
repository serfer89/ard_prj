...
void setup_wifi() {
  ...
  {digitalWrite(led_1, HIGH); client.publish(room, "o/1/2/1"); but1_state = HIGH; Serial.println("btn 1 - HIGH");}
  
  if (digitalRead(but_1) == LOW )
  {digitalWrite(led_1, LOW); client.publish(room, "o/1/2/0"); but1_state = LOW; Serial.println("btn 1 - LOW");}
  }
  ...
  {digitalWrite(led_2, HIGH); client.publish(room, "o/1/3/1"); but2_state = HIGH; Serial.println("btn 2 - HIGH");}
  
  if (digitalRead(but_2) == LOW )
  {digitalWrite(led_2, LOW); client.publish(room, "o/1/3/0"); but2_state = LOW; Serial.println("btn 2 - LOW");}
  }
 ...
 {digitalWrite(led_3, HIGH); client.publish(room, "o/1/4/1"); but3_state = HIGH; Serial.println("btn 3 - HIGH");}
  
  if (digitalRead(but_3) == LOW )
  {digitalWrite(led_3, LOW); client.publish(room, "o/1/4/0"); but3_state = LOW; Serial.println("btn 3 - LOW");}
  }
...
bool reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish("Кухня", "hello world");
      // ... and resubscribe
...
*/
 ...
 long now = millis();
  if (now - lastMsg > 20000) {
    ...
    client.publish("Кухня/ip",  ip.c_str());
