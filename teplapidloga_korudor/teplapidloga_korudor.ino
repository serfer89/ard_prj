...
switch (payload[0])
    {
    case '1':
   if (payload[2] == '0' ) { digitalWrite(D4, LOW);} 
...
String sv = "s/2/1/"+ String(state);// dev state     
    ...
    Serial.println(temp);
    if (temp > 25 && state == 1) {Serial.println("OFF"); state=0;
        client.publish("Коридор", "1/0/3");
}
    if (temp < 25 && state == 0) {Serial.println("ON"); state=1;
        client.publish("Коридор",  "1/1/3");
}
    client.publish("Коридор/Температура",  String(temp).c_str(),TRUE);
    client.publish("Коридор2/ip",  ip.c_str(),TRUE);
